...
(36 modules; entry `src/main.js`). The old monolithic `game.js` is gone — it was split by
...
2. **`REGRESSION_CHECKLIST.md`** — the manual QA pass for everything the golden suite can't
...
- `build.mjs` deliberately excludes `AGENTS.md`, docs, `test/`, and `tools/` from `dist/`.
